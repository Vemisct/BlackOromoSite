...
app.use(async (req, res, next) => {
    if (req.user && req.user.id) {
        try {
            await pool.query(`
                INSERT INTO user_sessions (user_id, user_email, last_active)
...
app.post('/api/reviews', async (req, res) => {
    ...
    try {
        await pool.query(
            `INSERT INTO reviews (user_name, user_email, rating, text, approved) VALUES (?, ?, ?, ?, 0)`,
            [req.user.name, req.user.email, rating, text]
        );
...
app.get('/api/admin/stats', isAdmin, async (req, res) => {
    try {
        ...
        res.json({
            views: views,
            active_users: active[0].active_users || 0,
            total_reviews: totalReviews[0].total_reviews || 0,
            approved_reviews: approvedReviews[0].approved_reviews || 0
        });
