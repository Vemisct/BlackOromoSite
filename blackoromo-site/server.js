...
// Сесії (можна потім додати connect-pg-simple, поки залишимо MemoryStore)
app.use(session({
...
async function initTables() {
    ...
    try {
        ...
        console.log('✅ Таблиці створено (PostgreSQL)');
    } catch (err) {
        console.error('Помилка ініціалізації таблиць:', err);
    } finally {
        client.release();
    }
}
...
app.get('/api/admin/stats', isAdmin, async (req, res) => {
    try {
        const viewsResult = await pool.query(`SELECT DATE(timestamp) as date, COUNT(*) as count FROM page_views WHERE timestamp >= NOW() - INTERVAL '7 days' GROUP BY DATE(timestamp) ORDER BY date ASC`);
        const activeResult = await pool.query(`SELECT COUNT(DISTINCT user_id) as active_users FROM user_sessions WHERE last_active >= NOW() - INTERVAL '1 day'`);
        const totalReviewsResult = await pool.query(`SELECT COUNT(*) as total_reviews FROM reviews`);
        const approvedReviewsResult = await pool.query(`SELECT COUNT(*) as approved_reviews FROM reviews WHERE approved = 1`);
        res.json({
            views: viewsResult.rows,
            active_users: activeResult.rows[0].active_users || 0,
            total_reviews: totalReviewsResult.rows[0].total_reviews || 0,
            approved_reviews: approvedReviewsResult.rows[0].approved_reviews || 0
        });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/admin/reviews', isAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT * FROM reviews ORDER BY timestamp DESC`);
        res.json(result.rows);
    } catch(e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/admin/reviews/:id/approve', isAdmin, async (req, res) => {
    try {
        await pool.query(`UPDATE reviews SET approved = 1 WHERE id = $1`, [req.params.id]);
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/admin/messages', isAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT * FROM messages ORDER BY timestamp DESC LIMIT 200`);
        res.json(result.rows);
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/admin/menu', isAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO menu (category, name, price, description) VALUES ($1, $2, $3, $4) RETURNING id`, [category, name, price, description]);
...
app.put('/api/admin/menu/:id', isAdmin, async (req, res) => {
    const { category, name, price, description } = req.body;
    try {
        await pool.query(`UPDATE menu SET category=$1, name=$2, price=$3, description=$4 WHERE id=$5`, [category, name, price, description, req.params.id]);
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
