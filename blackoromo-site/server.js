...
async function createPool() {
    try {
        // Якщо є DATABASE_URL – використовуємо його (найпростіше)
        if (process.env.DATABASE_URL) {
            pool = mysql.createPool(process.env.DATABASE_URL);
            const conn = await pool.getConnection();
            console.log('✅ Підключено до MySQL через DATABASE_URL');
            conn.release();
            return;
        }

        // Інакше – окремі змінні (для локального тесту або fallback)
        let caCert = null;
        try {
        ...
        const conn = await pool.getConnection();
        console.log('✅ Підключено до MySQL через окремі змінні');
        conn.release();
    } catch (err) {
